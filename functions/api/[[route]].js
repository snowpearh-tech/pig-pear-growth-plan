import { categoryInputSchema, createTransactionInputSchema, historyQuerySchema, loginInputSchema, quoteInputSchema, redeemInputSchema, rewardInputSchema, ruleInputSchema, settingsInputSchema, levelInputSchema, } from "@shared/contracts";
import { clearSessionCookie, createSessionCookie, isAuthenticated, requireAuth, } from "../_lib/auth";
import { ApiError, handleError, json, parseBody, requireNumericId } from "../_lib/http";
import { createCategory, createLevel, createQuote, createReward, createRule, createTransactionFromInput, deleteCategory, deleteLevel, deleteQuote, deleteReward, deleteRule, getState, isDatabaseReady, listTransactions, redeemReward, seedDatabase, updateCategory, updateLevel, updateQuote, updateReward, updateRule, updateSettings, } from "../_lib/repository";
...
export const onRequest = async (context) => {
    ...
    try {
        ...
        if (resource === "rewards" && request.method === "POST") {
            await requireAuth(request, env);
            return json(await createReward(env, await parseBody(request, rewardInputSchema)), { status: 201 });
        }
        if (resource === "rewards" && idSegment && request.method === "PUT") {
            await requireAuth(request, env);
            return json(await updateReward(env, requireNumericId(idSegment), await parseBody(request, rewardInputSchema)));
        }
        ...
        if (resource === "quotes" && idSegment && request.method === "PUT") {
            await requireAuth(request, env);
            return json(await updateQuote(env, requireNumericId(idSegment), await parseBody(request, quoteInputSchema)));
        }
        ...
        if (resource === "categories" && request.method === "POST") {
            await requireAuth(request, env);
            return json(await createCategory(env, await parseBody(request, categoryInputSchema)), { status: 201 });
        }
        if (resource === "categories" && idSegment && request.method === "PUT") {
            await requireAuth(request, env);
            return json(await updateCategory(env, requireNumericId(idSegment), await parseBody(request, categoryInputSchema)));
        }
        ...
        if (resource === "levels" && idSegment && request.method === "PUT") {
            await requireAuth(request, env);
            return json(await updateLevel(env, requireNumericId(idSegment), await parseBody(request, levelInputSchema)));
        }
        ...
        if (resource === "settings" && request.method === "PUT") {
            await requireAuth(request, env);
            return json(await updateSettings(env, await parseBody(request, settingsInputSchema)));
        }
